...
async function saveOrderToSheet(order) {
  try {
    ...
    console.log('Orders sheet error', e);
...
function orderStatusOptionsHTML(current) {
  const opts = ['قيد المعالجة','تم التأكيد','تم الشحن','تم التسليم','ملغي'];
...
function renderOrdersTable(list) {
  ...
  <div class="table-wrap">
    <table>
      <thead>
        <tr><th>رقم الطلب</th><th>التاريخ</th><th>الزبون</th><th>المدينة</th><th>المنتجات</th><th>الإجمالي</th><th>الحالة</th></tr>
      </thead>
      <tbody>
        ${list.map((o, i) => {
          let itemsText = '';
          if (typeof o.items === 'string') {
            itemsText = o.items.split('|').map(s => s.trim()).filter(Boolean).join('، ');
          } else if (Array.isArray(o.items)) {
            itemsText = o.items.map(it => `${it.name} ×${it.qty}`).join('، ');
          }
          const shortItems = itemsText.length > 36 ? itemsText.slice(0,36) + '…' : itemsText;
          const phoneClean = (o.phone||'').replace(/[^0-9+]/g,'');
          const rowNum = o.row || '';
          const orderLabel = o.orderId || ('#' + (list.length - i));
          return `
          <tr>
            ...
            <td>
              ...
              <a href="tel:${phoneClean}" style="color:var(--blue-main);font-size:0.82rem;">${o.phone||''}</a>
              <a href="https://wa.me/${phoneClean.replace(/^0/,'212')}" target="_blank" style="color:var(--success);margin-right:6px;"><i class="fab fa-whatsapp"></i></a>
            </td>
            <td>${o.city || '—'}</td>
            <td title="${(itemsText||'').replace(/"/g,'&quot;')}">${shortItems || '—'}</td>
            <td><strong>${o.total||0} د.م</strong></td>
            <td>
              <select class="order-status-select" data-row="${rowNum}" onchange="changeOrderStatus(this)">
...
async function changeOrderStatus(selectEl) {
  ...
  if (!row) { toast('ماقدرش نبدل الحالة، جددي الصفحة وحاولي مرة أخرى ❌','danger'); return; }
  const res = await apiCall('updateOrderStatus', { row, status });
  if (res && res.status !== 'error') {
    toast('تم تحديث حالة الطلب ✓','success');
  } else {
    toast('مشكل فتحديث الحالة، حاولي مرة أخرى ❌','danger');
  }
}
...
async function trackOrderByPhone(){
  ...
  box.innerHTML = '<div style="text-align:center;padding:20px;"><i class="fas fa-spinner fa-spin" style="font-size:1.6rem;color:var(--blue-main);"></i></div>';
  ...
  try {
    ...
    if(!matches.length){
      box.innerHTML = `<div style="text-align:center;padding:20px;color:var(--text-light);">
        <i class="fas fa-box-open" style="font-size:2rem;margin-bottom:8px;display:block;opacity:0.4;"></i>
    ...
    box.innerHTML = matches.map(o=>{
      ...
      return `<div class="order-card" style="margin-bottom:10px;">
        <div class="order-card-header">
          <span class="order-num">${o.date||''}</span>
          <span class="order-total-badge" style="background:var(--success);">${status}</span>
        </div>
        <div class="order-card-body" style="grid-template-columns:1fr;">
          <div class="order-info-block">
            <h5>المنتجات</h5>
            <p style="font-size:0.85rem;">${items}</p>
            <p style="margin-top:6px;"><strong>${o.total||0} درهم</strong></p>
          </div>
        </div>
      </div>`;
    }).join('');
...
async function sendOrder(){
  ...
  const honeypot=(document.getElementById('ordWebsite')||{}).value||'';
  if(!name||!phone||!city||!addr){toast('يرجى ملء جميع الحقول المطلوبة ♡','danger');return;}
  if(!cart.length){toast('السلة فارغة ♡','danger');return;}

  const sendBtn = document.querySelector('#checkoutModal .send-btn');
  if(sendBtn){ sendBtn.disabled = true; sendBtn.style.opacity = '0.6'; }

  const finalNotes = coupon ? `[كود: ${coupon}] ${notes}` : notes;
  const res = await saveOrderToSheet({
    name, phone, city, address: addr, notes: finalNotes,
    items: [...cart], website: honeypot
  });

  if(sendBtn){ sendBtn.disabled = false; sendBtn.style.opacity = '1'; }

  if (!res || res.status === 'error') {
    if (res && res.message === 'insufficient_stock') {
      const details = (res.details||[]).map(d=>`${d.name} (باقي ${d.available} فقط)`).join('، ');
      toast(`الكمية غير متوفرة: ${details} ❌`,'danger');
    } else if (res && res.message === 'invalid_data') {
      toast('المعلومات المدخلة غير صحيحة، تأكدي منها وحاولي مرة أخرى ❌','danger');
    } else {
      toast('تعذر إرسال الطلب، تأكدي من الاتصال بالإنترنت وحاولي مرة أخرى ❌','danger');
    }
    return;
  }

  const orderId = res.orderId || '';
  const serverTotal = (res.total !== undefined && res.total !== null) ? res.total : cartTotal();

  let msg=`🌸 *طلب جديد - SAMA CROCHET* 🌸\n\n`;
  if(orderId) msg+=`🔖 *رقم الطلب:* ${orderId}\n`;
  msg+=`👤 *الاسم:* ${name}\n📞 *الهاتف:* ${phone}\n🏙️ *المدينة:* ${city}\n📍 *العنوان:*\n${addr}\n`;
  if(coupon) msg+=`🎟️ *كود الخصم:* ${coupon}\n`;
  if(notes) msg+=`📝 *ملاحظات:* ${notes}\n`;
  msg+=`\n🛍️ *المنتجات:*\n`;
  cart.forEach(it=>{ msg+=`• ${it.name} × ${it.qty} = ${it.price*it.qty} درهم\n`; });
  msg+=`\n💰 *الإجمالي: ${serverTotal} درهم*`;
  if(coupon) msg+=` (قبل تطبيق كود الخصم — سيتم تأكيد السعر النهائي معك)`;
  msg+=`\n\nشكراً لثقتك بـ SAMA CROCHET ♡`;

  const order={
    id: orderId || Date.now(), name, phone, city,
    address: addr, notes: finalNotes,
    items:[...cart], total: serverTotal,
...
function adminTab(name,btn){
  ...
  if(name==='overview'){ loadOverview(); loadStats(); }
  if(name==='products') loadProductsTable();
  if(name==='orders') loadOrders();
  if(name==='stock') loadStock();
}

function loadAdminData(){loadOverview();loadStats();}
...
async function loadStats(){
  ...
  set('st-revenue-day', s.dailyRevenue||0);
  set('st-revenue-month', s.monthlyRevenue||0);
  set('st-revenue-total', s.totalRevenue||0);
  set('st-customers', s.uniqueCustomers||0);

  const tb=document.getElementById('bestSellersTable');
  if (tb) {
    if (s.bestSellers && s.bestSellers.length) {
      tb.innerHTML = s.bestSellers.map(b=>`<tr><td><strong>${b.name}</strong></td><td>${b.qty}</td></tr>`).join('');
    } else {
      tb.innerHTML = '<tr><td colspan="2" style="text-align:center;color:var(--text-light);">لا توجد مبيعات بعد</td></tr>';
    }
  }
}
